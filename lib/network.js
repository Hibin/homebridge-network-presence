...
const init = function() {
    ...
    this.log(`Check if hostnames resolution is required...`)
    // check if some device has hostname defined and it is not empty
    const skipNameResolution = this.devicesConfig.find(device => device.hostname) ? false : true;
    this.log(`Hostname resolution is ${skipNameResolution ? 'disabled' : 'enabled'}`)
...
class NetworkObserver extends EventEmitter {
    constructor(interval, range, skipNameResolution) {
        super();
        this.pingOptions = {
            timeout: 2000
        };
        ...
        this.range = range;
        this.skipNameResolution = skipNameResolution;
        this.cachedDevices = [];
        this.tick();
    }
    ...
    get devices() {
        return find({
            address: this.range,
            skipNameResolution: this.skipNameResolution,
        })
    }
